...
for i in name_list: 
  full_name = find(i+'.mtx', './user-data/benchmarks/')
  if full_name:
    print(full_name)
  else:
    continue
...
ell_reuse = np.concatenate([df['ell_reuse_distance'], df_wasm['ell_reuse_distance']], axis = 0)
min_ell_reuse = ell_reuse.min()
max_ell_reuse = ell_reuse.max()
print min_ell_reuse
print max_ell_reuse
...
plt.scatter(df['ws'], df['mflops'], label='ELL C', c=df['ell_reuse_distance'], cmap='gnuplot2', s=12**2, alpha=0.5, marker='o')
plt.clim(0, 100)
plt.scatter(df_wasm['ws'], df_wasm['mflops'], label='ELL WebAssembly', c=df_wasm['ell_reuse_distance'], cmap='gnuplot2', s=12**2, alpha=0.5, marker='^')
plt.clim(0, 100)
cbar = plt.colorbar()
